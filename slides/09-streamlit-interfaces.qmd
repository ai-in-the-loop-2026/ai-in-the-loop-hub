...
::: {style="text-align: center; font-size: 1.3em; line-height: 2.2; margin-top: 1em;"}
**Script runs top to bottom** &ensp;→&ensp; **UI renders in browser**

&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;↑ &emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; ↓

**triggers rerun** &emsp;&ensp;←&ensp; **User interacts (click, type)**
:::
...
```python
        # Inside the button handler, after invoke() returns:
...
Color-coded output: `st.success` (green), `st.warning` (yellow), `st.error` (red).
...
::: {style="text-align: center;"}
![](images/09-paper-analyzer-ui.png){height=550px}
:::

<!-- IMAGE: ![](images/09-paper-analyzer-ui.png){height=550px}
...
## Node 1: Parse URL and fetch metadata {.smaller}
...
## Nodes 3a and 3b: Summarize or fallback {.smaller}
...
for event in pipeline.stream({"arxiv_url": url, ...}, stream_mode="updates"):
    node_name = list(event.keys())[0]
    print(f"Completed: {node_name}")
```

| | `invoke()` | `stream(stream_mode="updates")` |
|---|---|---|
...
## Full Example 2: Setup and streaming {.smaller}
    ...
    if not url.strip():
        ...
        with st.status("Analyzing paper...", expanded=True) as status:
            ...
            for event in pipeline.stream(initial_state, stream_mode="updates"):
                ...
                final_state.update(event[node_name])
            status.update(label="Analysis complete!", state="complete")
        # ... display results (next slide)
```

---

## Full Example 2: Displaying results

```python
        # Still inside the button handler, after streaming:
        st.markdown("### Summary")
...
Combines all the display elements from the previous slides into one complete app.
...
Regular variables reset on every rerun. Session state persists.
...
## Example 3: The chat loop {.smaller}
