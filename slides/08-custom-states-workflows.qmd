...
**Why LLM-driven?** The URL format varies (`arxiv.org/abs/...`, `arxiv.org/pdf/...`, bare ID). The LLM parses flexibly; the loop lets it retry on errors.

**Why a manual loop instead of `ToolNode`/`tools_condition`?**

- This is a **data processing pipeline** with custom state—not a conversational agent
- Only **one node** needs tool retry; the rest are simple
- Messages stay **local to this node**, not in pipeline state
- Keeps the **graph structure simple** (no subgraphs or extra conditional edges)

Use deck 05's pattern for full agentic workflows. Use this pattern when one node in a larger pipeline needs flexible tool use.
...
No tool needed here—just LLM analysis of the extracted text.
...
def get_impact(state: PaperState) -> dict:
    ...
    # Direct call—arxiv_id is already clean in state, no LLM parsing needed
    result = get_paper_impact.invoke({"arxiv_id": state["arxiv_id"]})
    impact = json.loads(result)
    return {"impact": impact}
```

**New pattern**: A node that always runs a tool directly—no LLM deciding whether or how to call it. Works when input is already structured and the call is unconditional.
