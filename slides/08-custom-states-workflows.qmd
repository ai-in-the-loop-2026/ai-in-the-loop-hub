...
The pattern: user message → AI response → (tool call → tool result →)* AI response
...
1. Extract metadata from a research paper
2. Verify citations against a database
3. Analyze content for key themes
...
**Goal**: Process research papers through multiple analysis stages.

```python
from typing_extensions import TypedDict

class DocumentState(TypedDict):
    document: str           # Input: the raw document text
    metadata: dict | None   # Extracted title, authors, date
    citations: list[str]    # Verified citation references
    analysis: str | None    # Content analysis
...
::: {style="text-align: center;"}
![](images/08-doc-pipeline.png){width=75%}
:::

<!-- IMAGE: ![](images/08-doc-pipeline.png){width=75%}

     DESCRIPTION: LangGraph-style horizontal flow diagram for document processing.

     PROMPT FOR NANO BANANA PRO: Create a LangGraph-style horizontal flow diagram. Show:
     START (small circle) → "extract_metadata" (rounded box) → "verify_citations" (rounded box
     with small wrench/tool icon) → "analyze" (rounded box) → "summarize" (rounded box) →
     END (small circle). Above the chain, show a "DocumentState" box with fields: document,
     metadata, citations, analysis, summary. Draw a dotted line from state to nodes. Label
     the verify_citations edge with a small tool icon. Clean style with soft colors. -->

Four specialized nodes, each updating specific state fields.
...
def extract_metadata(state: DocumentState) -> dict:
    """Extract title, authors, and date from the document."""
    prompt = f"""Extract metadata from this document.
Return as JSON with keys: title, authors (list), date.

Document:
{state['document'][:2000]}"""  # Use beginning of doc

    response = llm.invoke([HumanMessage(content=prompt)])
    # In practice, use with_structured_output() for reliable parsing
    import json
    metadata = json.loads(response.content)
    return {"metadata": metadata}
```

Returns only the field it updates—LangGraph merges it into state.

---

## Node 2: Verify citations with tools
...
@tool
def lookup_citation(doi: str) -> str:
    """Look up a citation by DOI to verify it exists."""
    # In a real app: call CrossRef API
    return f"Found: {doi} - Verified as valid reference"

def verify_citations(state: DocumentState) -> dict:
    """Extract and verify DOIs from the document."""
    llm_with_tools = llm.bind_tools([lookup_citation])

    prompt = f"""Find DOIs in this document and call lookup_citation for each.

Document:
{state['document']}"""

    response = llm_with_tools.invoke([HumanMessage(content=prompt)])
    # For simplicity, assume model extracts DOIs in its response
    # In practice, you'd handle the tool call loop as in deck 05
    return {"citations": ["doi:10.1234/example"]}  # Simplified
```

---

## Nodes 3 and 4: Analyze and summarize

```python
def analyze_content(state: DocumentState) -> dict:
    """Analyze document themes using extracted metadata."""
    prompt = f"""Analyze the key themes and contributions of this paper.
...
def summarize(state: DocumentState) -> dict:
    ...
    prompt = f"""Write a comprehensive summary.

Metadata: {state['metadata']}
Verified citations: {state['citations']}
...
def build_document_pipeline():
    ...
    graph.add_edge("extract_metadata", "verify_citations")
    graph.add_edge("verify_citations", "analyze")
...
def analyze(
    ...
    """Analyze a research paper and generate a summary."""
...
```bash
$ python analyze.py paper.txt -o summary.txt
Input: paper.txt
Output: summary.txt
...
```bash
python analyze.py paper.txt --verbose   # verbose is True
python analyze.py paper.txt             # verbose is False
```
