...
def get_impact(state: PaperState) -> dict:
    ...
    return {
        "metadata": updated,
        "impact": {"citations": data["citations"], "venue": data["venue"], "doi": data["doi"]}
    }
```
...
That's why we do `state["metadata"].copy()` then modify—if we just returned new metadata, we'd lose the `key_contributions` from Node 2.
...
This sequential approach works, but is **inefficient** for papers indexed by Semantic Scholar—we run the LLM extraction even when API data is available.

A **more efficient pattern** would use conditional edges:

1. Try Semantic Scholar first
2. If found → skip LLM, use API metadata directly
3. If not found → fall back to LLM extraction (e.g., for new papers)

This requires a more complex graph structure (covered in Example 2). Here we prioritize simplicity: always extract with LLM, then overwrite with API data when available.
