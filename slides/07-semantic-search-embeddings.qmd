...
::: {.callout-note}
`langchain_classic` is a legacy package (maintained until Dec 2026) that contains `EnsembleRetriever` with built-in Reciprocal Rank Fusion. The modern alternative is to use `RunnableParallel` from `langchain_core` and implement RRF yourself.
:::
...
- [EnsembleRetriever (langchain-classic)](https://reference.langchain.com/python/langchain_classic/retrievers/)
